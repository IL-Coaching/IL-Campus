...
import { obtenerFuerzaMaximaCliente } from "@/nucleo/acciones/metricas.accion";
import { Loader2, Dumbbell, TrendingUp } from "lucide-react";
import { PorcentajesCliente } from "@prisma/client";

interface Props {
    clienteId: string;
}

type FuerzaRecord = PorcentajesCliente & {
    ejercicio: { nombre: string; musculoPrincipal: string };
};

export default function MetricasFuerza({ clienteId }: Props) {
    const [loading, setLoading] = useState(true);
    const [records, setRecords] = useState<FuerzaRecord[]>([]);

    useEffect(() => {
        async function fetchFuerza() {
            setLoading(true);
            const res = await obtenerFuerzaMaximaCliente(clienteId);
            if (res.exito && res.datos) {
                setRecords(res.datos as FuerzaRecord[]);
            }
            setLoading(false);
        }
        fetchFuerza();
    ...
    if (loading) {
        return (
            <div className="flex justify-center items-center py-12">
                <Loader2 className="animate-spin text-naranja" size={32} />
            </div>
        );
    }
    ...
    return (
        <div className="bg-marino-2 border border-marino-4 p-8 rounded-3xl mt-8">
            <div className="flex items-center gap-3 mb-6">
                <div className="p-3 bg-naranja/10 border border-naranja/20 rounded-2xl">
                    <TrendingUp className="text-naranja" size={24} />
                </div>
                <div>
                    <h3 className="text-xl font-barlow-condensed font-black uppercase text-blanco leading-none">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {records.map((record) => (
                    <div key={record.id} className="bg-marino-3/50 border border-marino-4 rounded-2xl p-5 hover:border-naranja/40 transition-colors">
                        ...
                        <div className="flex items-end gap-2">
                            <span className="text-4xl font-barlow-condensed font-black text-blanco leading-none">
