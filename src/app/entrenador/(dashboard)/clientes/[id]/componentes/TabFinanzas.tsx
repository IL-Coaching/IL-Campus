...
export default function TabFinanzas({ clienteId, clienteNombre }: Props) {
    ...
    useEffect(() => {
        const fetchFinanzas = async () => {
            ...
            setResumen(data as ResumenFinanciero);
            setLoading(false);
        };
        fetchFinanzas();
    }, [clienteId]);
    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center p-20 space-y-4">
                <div className="w-12 h-12 border-4 border-naranja/20 border-t-naranja rounded-full animate-spin"></div>
                <p className="text-xs font-black text-gris uppercase tracking-widest">Calculando Balance Económico...</p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-8 fade-up visible pb-12">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-6">
                <div>
                    <h3 className="text-xl md:text-2xl font-barlow-condensed font-black uppercase text-blanco tracking-tight leading-none">
                ...
                <button
                    ...
                    className="w-full md:w-auto flex items-center justify-center gap-2 px-8 py-4 md:py-3 bg-gradient-to-r from-verde to-emerald-500 hover:from-emerald-500 hover:to-verde text-marino font-black rounded-2xl text-[0.65rem] uppercase tracking-widest transition-all shadow-xl shadow-verde/20 active:scale-95"
                >
            ...
            <div className={`p-6 md:p-8 rounded-3xl border flex flex-col md:flex-row items-start md:items-center justify-between shadow-2xl overflow-hidden relative group transition-all duration-500 ${resumen?.estado === 'AL_DIA' ? 'bg-verde/5 border-verde/20' :
                ...
                <div className="flex items-center gap-4 md:gap-6 relative z-10 w-full md:w-auto">
                    <div className={`p-4 md:p-5 rounded-2xl ${resumen?.estado === 'AL_DIA' ? 'bg-verde/10 text-verde' :
                        resumen?.estado === 'VENCIDO' ? 'bg-rojo/10 text-rojo' : 'bg-marino-4 text-gris'
                        }`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
                <div className="bg-marino-2 border border-marino-4 p-5 md:p-6 rounded-2xl flex items-center gap-4 shadow-lg group hover:border-naranja/30 transition-all">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <div className="text-xl md:text-2xl font-barlow-condensed font-black text-blanco mt-1">${totalFacturado.toLocaleString()}</div>
                    </div>
                </div>

                <div className="bg-marino-2 border border-marino-4 p-5 md:p-6 rounded-2xl flex items-center gap-4 shadow-lg group hover:border-verde/30 transition-all">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <div className="text-xl md:text-2xl font-barlow-condensed font-black text-blanco mt-1">
                            {ultimoCobro ? `$${ultimoCobro.montoArs.toLocaleString()}` : "$0"}
                        </div>
                    </div>
                </div>

                <div className="bg-marino-2 border border-marino-4 p-5 md:p-6 rounded-2xl flex items-center gap-4 shadow-lg group hover:border-azul-claro/30 transition-all">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <div className="text-xl md:text-2xl font-barlow-condensed font-black text-blanco mt-1 truncate">
